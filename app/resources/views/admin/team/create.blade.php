...
@section('content')
    <div class="flex flex-col justify-center items-center">
        <h1 class="text-4xl text-custom-green text-center my-12">Add Team</h1>
        <form action="{{ route('addTeam') }}" class="w-2/5" method="post" enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
            ...
            @endError
            <div class="mb-4">
                <input type="text" name="role" placeholder="Role" class="bg-gray-00 border-2 border-custom-green w-full p-4 rounded-lg @error('role') border-red-500 @endError ">
            </div>
            @error('email')
            <div class="my-4">
                <p class="text-xl text-red-500">{{ $message  }}</p>
        ...
        <div class="w-full px-24 my-8">
                        ...
                        <tr>
                            <td class="p-2 whitespace-nowrap">
                                <div class="flex items-center py-4">
                                    <div class="flex-shrink-0 mr-2 sm:mr-3"><img class="rounded-full w-24 h-24" src="{{ asset('storage/'.$one->picture) }}" alt=""></div>
                                    <div class="font-medium text-gray-800">{{ $one->name }}</div>
                                </div>
                            </td>
                            <td class="p-2 whitespace-nowrap">
                                <div class="text-left">{{ $one->role }}</div>
                            </td>
                            <td class="p-2 whitespace-nowrap">
                                <form action="{{ route('deleteTeam', $one) }}" method="post">
                                    @csrf
                                    @method('DELETE')
            ...
            @else
                <div class="h-[43vh]">
                    <h1 class="text-2xl text-custom-green text-center my-12">No Team members Available to show</h1>
                </div>
            @endif
        </div>
    </div>
